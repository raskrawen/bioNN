...
function drawGUI() {
    ...
    pausedCheckbox = createCheckbox('Game', false);
    pausedCheckbox.position(distInGUI * 1 + 20, height + 20);
    pausedCheckbox.changed(startGame); //once mouse released
    // speed:
...
function initializeNeurons() {
    ...
    //inhib neurons:
    let k = 0;
    for (j = 3; j < difficultyLevel+3+random(1,4); j++) {
        neurons.push(new NeuronType(random(300, 600), random(100, 600), false));
...
function selectLevel() {
    ...
    let difficultyLevel = int(splitString[1]);
